...
function setGame() {
	...
	setTwo();
	setTwo(); // upon  start of game, two two tiles will appear
	...
	for(let r=0; r < rows; r++) {
		for(let c = 0; c < columns; c++) {
			if(board[r][c] == 2048 && is2048Exist == false) {
				...
				alert("You are a star! You got 4096! Awesome!");
				is4096Exist = true;
...
function hasLost() {
	for(let r = 0; r < rows; r++) {
		for (let c = 0; c < columns; c++) {
			...
			if(
				r > 0 && board[r-1][c] === currentTile || //compares currentTile and adjacent tile above
				...
				c < columns - 1 && board[r][c+1] === currentTile //compares currentTile to the tile to the right
				) {
				return false;
			}
		}
